-- Rename columns to match Google Ads API terminology

ALTER TABLE ads_v2.policy_timeline RENAME COLUMN old_status TO old_approval_status;
ALTER TABLE ads_v2.policy_timeline RENAME COLUMN new_status TO new_approval_status;
ALTER TABLE ads_v2.policy_timeline RENAME COLUMN policy_topics TO policy_topic_entries;

-- Recreate partial index with new column name
DROP INDEX IF EXISTS ads_v2.idx_log_disapprovals;
CREATE INDEX idx_log_disapprovals ON ads_v2.policy_timeline (new_approval_status, changed_at DESC)
...
BEGIN
  ...
  INSERT INTO ads_v2.policy_timeline (
    ...
    old_approval_status, new_approval_status, asset_content, asset_type,
    campaign_name, campaign_id, ad_group_name, ad_group_id, policy_topic_entries
  ) VALUES (
    'ASSET_IN_AD',
    NEW.customer_id,
    NEW.asset_id,
...
-- Recreate bootstrap with new column names
